...
public class ToyFigureController extends HttpServlet {
    ...
    private void updateToyFigure(HttpServletRequest request,HttpServletResponse response) throws IOException {
    	int id = Integer.parseInt(request.getParameter("id"));
    	String name = request.getParameter("name");
    	int spareParts = Integer.parseInt(request.getParameter("spareParts"));
    	boolean stand = Boolean.parseBoolean("stand");
    	...
    	if(rowEffected > 0 )
    		out.println("successfully updated result in database.");
    	else
    		out.print("Failed to update Toy Figure in Database");
    }
